...
Tests are organized into groups called "Test Groups" which are represented by a color coded square. A <span style="color: green">green</span> square indicates that all test in that group passed. A <span style="color: red">red</span> square indicates that at least one test in that group failed.  The number of tests allocated per test group can be set with the `groupSize` flag (see below). 
